...
// AddTaskConfig 表示添加备份任务的配置结构, 仅用于读取TOML配置文件
...
type AddTaskConfig struct {
	...
	MaxFileSize  string   `toml:"max_file_size" comment:"最大文件大小(可选, 超过此尺寸的文件不备份, 单位字节, 默认为0表示不限制; 常用换算: 1KB=1024, 1MB=1048576, 1GB=1073741824, 1TB=1099511627776)"` // 最大文件大小
	MinFileSize  string   `toml:"min_file_size" comment:"最小文件大小(可选, 小于此尺寸的文件不备份, 单位字节, 默认为0表示不限制; 常用换算: 1KB=1024, 1MB=1048576, 1GB=1073741824, 1TB=1099511627776)"` // 最小文件大小
}

// TaskConfig 表示备份任务的配置结构
type TaskConfig struct {
	Name         string   // 任务名称
	BackupDir    string   // 备份源目录
	StorageDir   string   // 备份存储目录
	RetainCount  int      // 保留备份文件的数量
	RetainDays   int      // 保留备份文件的天数
	Compress     bool     // 是否压缩
